...
//			// 农场区域
    ...
    public static function getFireinfo($totalData)
    {
        ...
        if($managementarea and is_array($managementarea)) {
            foreach ($managementarea as $value) {
            ...
            if($managementarea) {
                $all[] = Fireprevention::find()->where($where)->andFilterWhere(['management_area' => $managementarea])->count();
                $part[] = Fireprevention::find()->where($where)->andFilterWhere(['management_area' => $managementarea, 'finished' => 2])->count();
                $finished[] = Fireprevention::find()->where($where)->andFilterWhere(['management_area' => $managementarea, 'finished' => 1])->count();
            }
            return ['all' => $all, 'part' => $part, 'real' => $finished];
        }
        return ['all' => [], 'part' => [], 'real' => []];
    }
