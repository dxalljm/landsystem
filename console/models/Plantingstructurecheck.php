...
//     		$plantsum = 0;
//     		$goodseedsum = 0;
//     		$planting = Plantingstructurecheck::find()->where(['farms_id'=>$allid])->all();
...
//     	var_dump($userid);
//     	var_dump($where);
    	$Plantingstructurecheck = Plantingstructurecheck::find ()->where (['management_area' => $where])->all ();
    	$data = [];
    	foreach($Plantingstructurecheck as $value) {
...
// 			$farm = Farms::find()->where(['management_area'=>$value])->all();
    		...
    		foreach ($plantid['id'] as $val) {
    			$plantsum = 0.0;
    			$goodseedsum = 0.0;
    			$area = Plantingstructurecheck::find()->where(['management_area'=>$value,'plant_id'=>$val,'year'=>User::getYear($userid)])->sum('area');
...
//     	foreach ( $area as $key => $value ) {
    			...
    			$planting = Plantingstructurecheck::find()->where(['management_area'=>$area,'year'=>date('Y')])->all();
    			foreach ($planting as $v) {
