							...
							switch ($ttpoModel->actionname) {
								case 'farmstransfer':	//整体过户
									...
									if($old['state'] == 1) {
										...
										$collModel->copy($ttpoModel->newnewfarms_id,true);
									} else {
										$collModel = Collection::findOne($old['id']);
										$collModel->copy($ttpoModel->newnewfarms_id);
										$collModel->delete();
									}
									...
									$plan = c::find()->where(['farms_id'=>$ttpoModel->oldfarms_id,'year'=>User::getYear()])->one();
								...
								case 'farmssplit':	//部分新建
									...
									if($old['state'] == 1) {
										$collModel = Collection::findOne($old['id']);
										$collModel->copy($ttpoModel->oldnewfarms_id,true);
										$collModel->copy($ttpoModel->newnewfarms_id,true);
									} else {
										//如果没交费就新建交费数据
								...
								case 'farmstozongdi':	//部分合并
									...
									if(Collection::iscq($ttpoModel->oldfarms_id)) {
										$old = Collection::find()->where(['farms_id'=>$ttpoModel->oldfarms_id,'payyear'=>date('Y')])->one();
										$old->delete();
										Collection::newCollection($ttpoModel->oldnewfarms_id);
									}
									if(Collection::iscq($ttpoModel->newfarms_id)) {
										$new = Collection::find()->where(['farms_id'=>$ttpoModel->newfarms_id,'payyear'=>date('Y')])->one();
										$new->delete();
										Collection::newCollection($ttpoModel->newnewfarms_id);
									}
//									Collection::newCollectionTtpoarea($ttpoModel->newnewfarms_id,$old,$new,$ttpoModel->ttpoarea);
