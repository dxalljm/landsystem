...
//exit;
	...
	public function actionCollectionreset($id)
	{
		...
		$collection = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear])->count();
		if($collection == 1) {
			...
			$dck0 = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear,'dckpay'=>0])->one();
			if(empty($dck0)) {
				$model->ypayarea = $model->measure;
				$model->ypaymoney = $model->real_income_amount;
				$model->owe = $model->real_income_amount;
				$model->real_income_amount = 0.00;
				$model->dckpay = 0;
				$model->measure = 0.00;
				$model->save();
			} else {
				$dck0->ypayarea = (float)bcadd($dck0->ypayarea,$model->measure,2);
				$dck0->ypaymoney = (float)bcadd($dck0->ypaymoney,$model->real_income_amount,2);
				$dck0->owe = $dck0->ypaymoney;
				$dck0->real_income_amount = 0.00;
//				var_dump($dck0);exit;
				$dck0->save();
				$model->delete();
			}
//			$collection = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear])->one();
//			$collection->real_income_amount = (float)bcsub($collection->real_income_amount,$model->real_income_amount,2);
//			$collection->ypayarea = (float)bcadd($model->ypayarea,$collection->ypayarea,2);
//			$collection->ypaymoney = (float)bcadd($model->ypaymoney,$collection->ypaymoney,2);
//			$collection->owe = (float)bcadd($model->owe,$collection->owe,2);
//			$collection->measure = (float)bcsub($collection->measure,$model->measure,2);
//			$collection->save();
//			$model->delete();
	...
	public function actionCollectionsend2($farms_id,$year = null)
	{
		if(empty($year))
			$year = User::getYear();
		...
		$farm = Farms::findOne($farms_id);
		//获取最后一次缴费数据
		$collection = Collection::find()->where(['farms_id'=>$farms_id,'year'=>$year])->orderBy('id DESC')->one();
		$model = $this->findModel($collection['id']);
		$old_area = sprintf("%.2f", $collection->real_income_amount / PlantPrice::find()->where(['years' => $model->payyear])->one()['price']);
		$collectiondataProvider = Collection::find()->where(['farms_id' => $farms_id])->andWhere('payyear<=' . $year)->orderBy('id DESC')->all();
...
//		var_dump($farm);exit;
		...
		if(empty($collection)) {
			Collection::newCollection($farms_id);
		}
		$price = PlantPrice::find()->where(['years'=>$year])->one();
		if(!$price) {
			return $this->redirect(['error/error','msg'=>'对不起,财务还没有设置'.$year.'年度的缴费基数。请确认财务设置了缴费基数再试!']);
		...
		$model = $this->findModel($collection['id']);
		$old_area = sprintf("%.2f", $collection->real_income_amount / PlantPrice::find()->where(['years' => $model->payyear])->one()['price']);
		$real = Collection::find()->where(['farms_id'=>$farms_id,'payyear'=>$year])->sum('real_income_amount');
		$count =  Collection::find()->where(['farms_id'=>$farms_id,'payyear'=>$year])->count();
		$collectiondataProvider = Collection::find()->where(['farms_id' => $farms_id])->andWhere('payyear<=' . $year)->orderBy('payyear DESC')->all();
		if ($model->load(Yii::$app->request->post())) {
//			var_dump($model->real_income_amount);
//			var_dump($collection['ypaymoney']);exit;
//			if(bccomp($farm['contractarea'],$real) == 1) {
//			var_dump($model);exit;
			if(bccomp($model->amounts_receivable,$real) == 1) {
				$model->real_income_amount = (float)$model->real_income_amount;
				$model->ypayarea = abs(bcsub($collection['ypayarea'], $model->measure, 2));

				if($real > 0) {
					$newreal = bcadd($real,$model->real_income_amount,2);
					$model->ypaymoney = abs(bcsub($model->amounts_receivable, $newreal, 2));
				} else {
...
//				exit;
			...
			if($model->ypaymoney > 0) {
				...
				$collectionModel->amounts_receivable = $model->amounts_receivable;
				$collectionModel->real_income_amount = 0.00;
				$collectionModel->ypayarea = abs(bcsub($collection['ypayarea'] , $model->measure,2));
				$collectionModel->ypaymoney = $model->ypaymoney;
				$collectionModel->owe = $model->ypaymoney;
...
//				$newmodel->save();
	...
	public function actionCollectionsendajax($id)
	{
//		var_dump($year);exit;
		$model = $this->findModel($id);
		$farm = Farms::find()->where(['id'=>$model->farms_id])->one();
		$collection = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear])->orderBy('id DESC')->one();
		$old_area = sprintf("%.2f", $collection->real_income_amount / PlantPrice::find()->where(['years' => $model->payyear])->one()['price']);
		$collectiondataProvider = Collection::find()->where(['farms_id' => $model->farms_id])->andWhere('payyear<=' . $model->payyear)->orderBy('id DESC')->all();

		return $this->renderAjax('collectionsendajax', [
			'model' => $model,
			'year' => $model->payyear,
	...
	public function actionCollectionsendsave($value,$id)
	{
		$model = Collection::findOne($id);
		$farm = Farms::find()->where(['id'=>$model->farms_id])->one();
		$collection = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear])->orderBy('id DESC')->one();
		$real = Collection::find()->where(['farms_id'=>$model->farms_id,'payyear'=>$model->payyear])->sum('real_income_amount');
...
//		var_dump($from);exit;
//		if(bccomp($farm['contractarea'],$real) == 1) {
			$model->real_income_amount = (float)$from['Collection']['real_income_amount'];
			$model->ypayarea = abs(bcsub($model->ypayarea, $from['Collection']['measure'], 2));
			$model->ypaymoney = abs(bcsub($model->ypaymoney, $from['Collection']['real_income_amount'], 2));
//			$model->real_income_amount = $model->real_income_amount;
			...
			$model->dckpay = 1;
			$model->state = 0;
			$model->measure = $from['Collection']['measure'];
			$model->iscq = 1;
//		var_dump($model);exit;
			$state = $model->save();
			Logs::writeLogs('更新缴费数据', $model);
//		}
		if($model->owe > 0) {
			...
			$collectionModel->create_at = time();
			$collectionModel->update_at = $collectionModel->create_at;
			$collectionModel->payyear = $model->payyear;
			$collectionModel->farms_id = $farm['id'];
			$collectionModel->amounts_receivable = $model->amounts_receivable;
			$collectionModel->real_income_amount = 0;
			$collectionModel->ypayarea = abs(bcsub($collection['ypayarea'], $from['Collection']['measure'], 2));
			$collectionModel->ypaymoney = abs(bcsub($collection['ypaymoney'], $from['Collection']['real_income_amount'], 2));
			$collectionModel->owe = abs(bcsub($collection['owe'], $from['Collection']['real_income_amount'], 2));
			$collectionModel->measure = 0.00;
			$collectionModel->ypayyear = $model->payyear;
			$collectionModel->dckpay = 0;
			$collectionModel->state = 0;
			if ($model->payyear < (int)date('Y')) {
				$collectionModel->iscq = 1;
