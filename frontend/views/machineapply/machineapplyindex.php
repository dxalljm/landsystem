...
// 				            	var_dump($model);exit;
            ...
            [
                ...
                'options' => ['width'=>'160'],
                'value' => function($model) {
                    return Farms::findOne($model->farms_id)['contractnumber'];
                }
            ],
            [
...
//            ],
            [
                ...
                'options' => ['width'=>'160'],
                'value' => function($model) {
                    $mf = Machineapplymachine::findOne($model->machineoffarm_id);
                    if($mf) {
...
//            ],
            ...
            [
                ...
                'value' => function($model) {
                    ...
                    switch ($model->state) {
                        case 0:
//                            $html .= Html::a('确认补贴金额', Url::to(['machineoffarm/machineoffarmcomparison', 'apply_id' => $model->id,'machineoffarm_id' => $model->machineoffarm_id]), ['class' => 'btn btn-xs btn-danger']);
                            $html .= '&nbsp;';
                            ...
                            break;
                        case 1:
//                            $html .= Html::a('确认补贴金额', '#', ['class' => 'btn btn-xs btn-success','disabled'=>'disabled']);
                            $html .= '&nbsp;';
                            $html .= Html::a('查看', Url::to(['machineapply/machineapplyprint', 'id' => $model->id,'farms_id'=>$model->farms_id]), ['class' => 'btn btn-xs btn-success']);
                            if($model->scanfinished) {
                                $class = 'btn btn-xs btn-success';
                            } else {
                                $class = 'btn btn-xs btn-danger';
                            }
                            $html .= '&nbsp;';
                            $html .= Html::a('扫描相关证件',Url::to(['photograph/photographmachine','apply_id'=>$model->id]),['class' => $class]);
                            if($model->scanfinished) {
                                $html .= '&nbsp;';
