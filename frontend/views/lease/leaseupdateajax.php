...
<?php //$farms = Farms::find()->where(['id'=>$_GET['farms_id']])->one();?>
    ...
    <table class="table table-bordered table-hover">
        ...
        <tr>
            <td align="right" width="20%"><strong>所属银行</strong></td>
            <td width="15%" align="center" >大兴安岭农村商业银行</td>
            <td width="8%" align="right" colspan="2"><strong>银行卡号</strong></td>
            <td width="" colspan="4"><?= Html::textInput('Lease[accountnumber]',$bank['accountnumber'],['id'=>'lease-accountnumber','class'=>'form-control']) ?></td>
        </tr>
        <tr>
        ...
        <tr><?php if($model->lease_area == '') $model->lease_area = $farm->contractarea - $overarea;?>
            ...
            <?php
            if($model->begindate == '') {
                $model->begindate = date('Y').'-01-01';
            }
            if($model->enddate == '') {
                $model->enddate = date('Y').'-12-31';
            }
            ?>
        ...
        <tr>
            ...
            <td align="right" width="10%"><strong>租金交纳方式</strong></td>
        ...
        <tr>
            ...
            <td colspan="6" align="left">
                <table>
                    <?php
//                    var_dump(\app\models\Subsidyratio::find()->where(['farms_id'=>$_GET['farms_id'],'lease_id'=>$model->id])->all());exit;
                    foreach(\app\models\Subsidyratio::find()->where(['farms_id'=>$_GET['farms_id'],'lease_id'=>$model->id])->all() as $key => $value) {
                        ...
                        echo '</tr>';
                        echo '<script>';
                        echo '$("#' . $type['mark'] . '_farmer").click(function(){var input = $(this).val();if(input == "100%") $(this).val("");});';
                        echo '$("#' . $type['mark'] . '_farmer").blur(function(){var input = $(this).val();if(input == "") $(this).val("100%");});';
                        echo '$("#' . $type['mark'] . '_farmer").change(function(){var input = $(this).val();if(Number(input)) {if(input>100) { alert("对不起,不能输入超过100的数值1。"); $(this).val("100%");$("#'.$type['mark'] . '_lessee").val("0%");} else {$(this).val(input+"%");getUNBFB("' . $type['mark'] . '_lessee",input);}} else {alert("只能输入数字。");$(this).val("100%");}});';
                        echo '$("#' . $type['mark'] . '_lessee").click(function(){var input = $(this).val();if(input == "0%") $(this).val("");});';
                        echo '$("#' . $type['mark'] . '_lessee").blur(function(){var input = $(this).val();if(input == "") $(this).val("0%");});';
                        echo '$("#' . $type['mark'] . '_lessee").change(function(){var input = $(this).val();if(Number(input)) {if(input>100) { alert("对不起,不能输入超过100的数值2。"); $(this).val("100%");} else {$(this).val(input+"%");getUNBFB("' . $type['mark'] . '_farmer",input);}} else {alert("只能输入数字。");$(this).val("0%");}});';
                        echo '</script>';
                    } ?>
                </table>
            </td>
