...
<script src="/front/vendor/bower/AdminLTE/plugins/input-mask/jquery.inputmask.js"></script>
<script src="/front/vendor/bower/AdminLTE/plugins/input-mask/jquery.inputmask.date.extensions.js"></script>
<script src="/front/vendor/bower/AdminLTE/plugins/input-mask/jquery.inputmask.extensions.js"></script>
...
<table class="table table-bordered table-hover">
		<tr>
			...
			<td align='left'><?= $form->field($model, 'farmname')->textInput(['maxlength' => 500])->label(false) ?></td>
			<td align='right'>承包人姓名</td>
			<td align='left'><?= $form->field($model, 'farmername')->textInput(['maxlength' => 500])->label(false) ?></td>
			<td align='right'>身份证号</td>
			<td colspan="3" align='left'><?= $form->field($model, 'cardid')->textInput(['maxlength' => 500])->label(false) ?></td>
		</tr>
		<tr>
			...
			<td align='left'><?= $form->field($model, 'longitude')->textInput(['maxlength' => 500,'data-inputmask'=>'"mask": "E999°99′99.99″"', 'data-mask'=>""])->label(false)->error(false) ?></td>
			<td align='right'>纬度</td>
			<td align='left'><?= $form->field($model, 'latitude')->textInput(['maxlength' => 500,'data-inputmask'=>'"mask": "N99°99′99.99″"', 'data-mask'=>""])->label(false)->error(false) ?></td>
		</tr>
			<tr>
    ...
    DateTimePicker::className(), [
		...
		<tr>
			<td width=15% align='right'>承包年限</td><?php if($model->begindate == '') $model->begindate = '2010-09-13';if($model->enddate == '') $model->enddate = '2025-09-13';?>
...
//							echo '<a href="#" id="zongdiinfo"><li class="select2-selection__choice" id="new' . Lease::getZongdi($zongdi) . '" title="' . $zongdi . '"><span class="remove text-red" role="presentation" onclick=zongdiRemove("' . Lease::getZongdi($zongdi) . '","' . Lease::getArea($zongdi) . '","dialog")>×</span>' . $zongdi . '</li></a>';
	...
	<span class="dropdown-wrapper" aria-hidden="true"></span>
		...
		<tr>
			...
			<td colspan="3" align='left'><?= $form->field($model, 'state')->radioList(Farms::getFarmsState([0,1,2,3,4,5]))->label(false)->error(false) ?></td>
		</tr>
		<tr>
			<td width=15% align='right'>冻结状态</td>
			<td colspan="7" align='left'><?= $form->field($model, 'locked')->radioList([0=>'未冻结',1=>'已冻结'])->label(false)->error(false) ?></td>
...
<script>
	$('#autoNumber').click(function(){
		$.get('index.php?r=farms/farmsautonumber', {farms_id: <?= $_GET['id']?>}, function (data) {
			$('#contract-number').val(data);
		});
	...
	$('#farms-state').change(function(){
		...
		if(val == 1 || val == 0 || val == 5) {
			var newcontractnumber = '<?= Farms::getNewContractserialnumber()?>' + '-' + '10' + '-' + arrayhth[2] + '-' + arrayhth[3];
		}
		if(val == 2) {
...
// 					var newnotstate = notstate*1 - Number(measure)*1;
...
// 		alert($('#farms-measure').val());
		if($('#farms-measure').val()*1 > contractarea) {
			if(measure > contractarea) {				
				...
				$('#farms-measure').val(newvalue.toFixed(2));
				$('#temp_measure').val(newvalue.toFixed(2));
				var newnotstate = $('#farms-measure').val() * 1 - contractarea * 1 + notclear * 1;
				$('#farms-notstate').val(newnotstate.toFixed(2));
			}
		} else {
// 			var newvalue = $('#farms-measure').val() * 1 - measure * 1;
...
// 								alert($('#farms-measure').val());
						...
						var farmMeasure = $('#farms-measure').val();
						//alert(farmMeasure);
// 						if(notclear > farmMeasure) {
// 							var newnotclear = notclear*1 - farmMeasure*1;
// 							$('#farms-notclear').val(newnotclear.toFixed(2));
...
// 								var newnotstate = oldnotstate*1 + measure - $('#farms-notclear').val();
// 								$('#farms-notclear').val(0);
// 								$('#farms-notstate').val(newnotstate.toFixed(2));
// 								if(getStateInfo())
// 									toHTH();
...
// 								alert($('#farms-measure').val());
								var oldnotstate = $('#farms-notstate').val();
								var newnotstate = oldnotstate*1 + $('#farms-measure').val()*1 - $('#farms-contractarea').val()*1;
								$('#farms-notclear').val(0);
								$('#farms-notstate').val(newnotstate.toFixed(2));
							}
...
//								$('#ymeasure').val(0);
	...
	function toHTH()
	{
		...
		var hth = $('#farms-contractnumber').val();
		var arrayhth = hth.split('-');
//		alert($('#farms-measure').val() +'-'+ $('#farms-notclear').val() +'-'+ $('#farms-notstate').val());
		var contractarea = $('#farms-measure').val()*1 + $('#farms-notclear').val()*1 - $('#farms-notstate').val()*1;
		arrayhth[2] = cutZero(contractarea.toFixed(2));
		$('#farms-contractnumber').val(arrayhth.join('-'));
		$('#farms-contractarea').val(arrayhth[2]);
