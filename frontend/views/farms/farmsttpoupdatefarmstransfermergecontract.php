							...
							<tr>
								<td width="46%" valign="top"><table width="100%" height="100%"
										...
										<tr><?= Html::hiddenInput('oldzongdi',$ttpoModel->oldzongdi,['id'=>'oldfarm-zongdi','class'=>'form-control']) ?>
											...
											<?php $ttpoarea = $ttpoModel->oldnotclear - $ttpoModel->ttpoarea;?>
											<?= Html::hiddenInput('changettpoarea',$ttpoarea,['id'=>'ttpozongdi-changearea'])?>
											<?= Html::hiddenInput('ttpoarea',$ttpoModel->ttpoarea,['id'=>'ttpozongdi-area']) ?>
											<td align='right' valign="middle">宗地面积</td><?= html::hiddenInput('tempoldmeasure',$ttpoModel->oldmeasure,['id'=>'temp_oldmeasure']) ?>
											<?= html::hiddenInput('tempoldnotclear',$ttpoModel->oldnotclear,['id'=>'temp_oldnotclear']) ?>
								...
								<td width="50%">
									<table width="99%" height="100%" class="table table-bordered table-hover">
										...
										<tr>
											...
											<td colspan="5" align='left'><?= $ttpoModel->newcontractnumber?></td>
										</tr>
										<tr>
											<td width=30% align='right'>新合同号</td><?php $newFarm->contractnumber = $ttpoModel->newchangecontractnumber;?>
										...
										<tr>
											...
											<td colspan="5" align='left'><?= $ttpoModel->newzongdi?></td>
										</tr>
										<tr>
											<td align='right'>转入宗地</td>
											<td colspan="5" align='left'><?= $ttpoModel->ttpozongdi?></td>
										</tr>
										<?= html::hiddenInput('tempzongdi','',['id'=>'temp-zongdi'])?><?php $newFarm->zongdi = $ttpoModel->newchangezongdi;?><?= $form->field($newFarm, 'zongdi')->hiddenInput()->label(false)->error(false) ?>
													...
													<span class="selection">
														<span class="select2-selection select2-selection--multiple" role="combobox" aria-autocomplete="list" aria-haspopup="true" aria-expanded="false" tabindex="0">
															<ul class="select2-selection__rendered">
																...
																if(is_array($ttpozongdi)) {
																	foreach ($ttpozongdi as $zongdi) {
										...
										<tr>
											...
											<?= html::hiddenInput('measure',$ttpoModel->newmeasure,['id'=>'ymeasure']) ?>
											<?= html::hiddenInput('tempnotclear',$ttpoModel->newnotclear,['id'=>'temp_notclear']) ?>
											<?= html::hiddenInput('tempnotstate',$ttpoModel->newnotstate,['id'=>'temp_notstate']) ?>
											<?php $newFarm->measure = $ttpoModel->newchangemeasure;?>
										...
										<tr>
											...
											<td colspan="5" align='left'><?= Farms::getContractnumberArea($ttpoModel->newcontractnumber) ?></td>
										</tr>
										<tr>
											<td align='right'>新合同面积</td><?PHP $newFarm->contractarea = Farms::getContractnumberArea($ttpoModel->newchangecontractnumber);?>
											<td colspan="5" align='left'><?= $form->field($newFarm, 'contractarea')->textInput(['readonly' => true])->label(false)->error(false) ?></td>
										</tr>
										<tr>
											<td align='right'>未明确地块面积</td><?php $newFarm->notclear = $ttpoModel->newchangenotclear;?>
...
<script>
//	function zongdisave(farms_id,zongdi,measure)
//	{
//		$str = zongdi+'('+measure+')';
//		$.getJSON("<?//= Url::to(['parce/parcelzongdioffarmsave'])?>//", {farms_id:farms_id,zongdiinfo: str});
//	}
...
//			alert(data.id);
	...
	function zongdiRemove(zongdi,measure,dialogID)
	{
		...
		$('#ttpozongdi-changearea').val($('#ttpozongdi-changearea').val()*1 + measure*1);
		$('#new'+zongdi).remove();
					...
					if(measure == '' || zongdi == '') {
						...
						if(measure > ymeasure) {
							...
							if(measure > $('#oldfarms-notclear').val()) {
								...
								var changettpoarea = $('#ttpozongdi-changearea').val();
								if(measure > changettpoarea) {
									alert('已经超过原合同面积，将自动截取为剩余面积。');
									zongdiForm(zongdi,changettpoarea);
									var newzongdi = zongdi+'('+changettpoarea+')';

									var newzongdihtml = '<li class="select2-selection__choice" id="new'+zongdi+'" title="'+newzongdi+'"><span class="remove text-red" role="presentation" onclick=zongdiRemove("'+zongdi+'","'+changettpoarea+'","dialog2")>×</span>'+newzongdi+'</li>';
									$('.select2-selection__rendered').append(newzongdihtml);

									var newvalue = $('#farms-measure').val()*1 + changettpoarea*1;

									$('#farms-measure').val(newvalue.toFixed(2));
									$('#temp_measure').val(newvalue.toFixed(2));
									$('#farms-notclear').val(<?= $ttpoModel->newnotclear?>);
									toHTH();
									$('#findZongdi').val('');
									$('#findMeasure').val('');
									var ttpozongdi = $('#ttpozongdi-zongdi').val();
									$('#ttpozongdi-zongdi').val(newzongdi+'、'+ttpozongdi);
									var ttpozongdi = $('#ttpozongdi-zongdi').val();
									...
									$('#ttpozongdi-area').val(ttpoarea*1 + changettpoarea*1);
									$('#ttpozongdi-changearea').val($('#ttpozongdi-changearea').val()*1 - changettpoarea*1);
								} else {
									...
									var noclearvalue = $('#temp_oldnotclear').val()*1 - measure*1;
...
// 					 	alert($('#zongdi').attr('value'));
	...
	$('#inputZongdi').dblclick(function(){
		var notclear = $('#ttpozongdi-changearea').val();
