...
<table
		...
		<tr>
			...
			<td align='left'><?= $form->field($model, 'latitude')->textInput(['maxlength' => 500,'data-inputmask'=>'"mask": "E99°99′99.99″"', 'data-mask'=>""])->label(false)->error(false) ?></td>
			</tr>
			<tr>
			...
			<td align='right'>合同号</td><?= Html::hiddenInput('temp_contractnumber',Farms::getNewContractnumber(),['id'=>'tempContractNumber'])?>
    ...
    DateTimePicker::className(), [
		...
		<tr>
			<td width=15% align='right'>承包年限</td><?php $model->begindate = date('Y-m-d');?>
...
//							echo '<a href="#" id="zongdiinfo"><li class="select2-selection__choice" id="new' . Lease::getZongdi($zongdi) . '" title="' . $zongdi . '"><span class="remove text-red" role="presentation" onclick=zongdiRemove("' . Lease::getZongdi($zongdi) . '","' . Lease::getArea($zongdi) . '","dialog")>×</span>' . $zongdi . '</li></a>';
	...
	<span class="dropdown-wrapper" aria-hidden="true"></span>
		...
		<tr>
		  ...
		  <td align='left'><?= $form->field($model, 'notclear')->textInput(['maxlength' => 500,'value'=>0])->label(false)->error(false) ?></td>
		  <td align='right'><?= $form->field($model, 'notstateinfo')->dropDownList(Farms::notstateInfo([6,7,8,9,10]))->label(false)->error(false)?></td>
		  <td align='left'><?= $form->field($model, 'notstate')->textInput(['maxlength' => 500,'value'=>0])->label(false)->error(false) ?></td>
    </tr>
		<tr>
