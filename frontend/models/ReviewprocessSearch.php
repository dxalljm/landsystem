...
//        var_dump($params);exit;
        if($params['r'] !== 'reviewprocess/reviewprocesscontractclaim')
...
//        var_dump($dataProvider->query->where);exit;
    ...
    {
//        var_dump(date('Y-m-d',$params['begindate']));
//        var_dump(date('Y-m-d',$params['enddate']));
//        exit;
        $query = Reviewprocess::find()->orderBy('id desc');
...
//        var_dump(Reviewprocess::find()->where(['state'=>4,'actionname'=>'loancreate','filereview'=>2])->all());
//        var_dump($dataProvider->query->where);exit;
        return $dataProvider;
    }
}
