...
//			return '<span class="notification">'.$result.'</span>';
	...
	public static function copy($farms_id,$model,$state=false)
	{
		...
		if($state) {
			...
			$model->payyear = $model->payyear;
			$model->farms_id = $farms_id;
			$model->billingtime = $model->billingtime;
			...
			$model->farms_id = $farms_id;
			$model->billingtime = $model->billingtime;
			$model->amounts_receivable = $model->amounts_receivable;
			$model->real_income_amount = $model->real_income_amount;
			$model->ypayyear = $model->ypayyear;
			$model->ypayarea = $model->ypayarea;
			$model->ypaymoney = $model->ypaymoney;
			$model->measure = $model->measure;
			$model->owe = $model->owe;
			$model->isupdate = $model->isupdate;
			$model->create_at = $model->create_at;
			$model->update_at = $model->update_at;
			$model->dckpay = $model->dckpay;
			$model->management_area = $model->management_area;
			$model->nonumber = $model->nonumber;
			$model->year = $model->year;
	...
	public static function newCollection($farms_id,$state = false)
	{
		...
		if($state) {
			...
			$model->state = 1;
			$model->farmstate = $farms['state'];
			$model->contractarea = $farms['contractarea'];
			$model->save();
		} else {
			...
			$collectionModel->farmstate = $farms['state'];
			$collectionModel->contractarea = $farms['contractarea'];
